...
import { IText, IIMage, ILine, IBlock } from '../type/canvas';
...
type IDrawSingleTextData = IText;
export function _drawSingleText(drawData: IDrawSingleTextData, drawOptions: IDrawOptions) {
  ...
  if (textDecoration !== 'none') {
    ...
    ctx.save();
    ctx.moveTo(toPx(x - 25), toPx(lineY));
    ctx.lineTo(toPx(x) + toPx(textWidth - 10), toPx(lineY));
...
export function drawText(params: IText, drawOptions: IDrawOptions) {
  ...
  if (Array.isArray(text)) {
    ...
    text.forEach(item => {
      ...
      const textWidth = _drawSingleText(Object.assign(item, { ...preText }), drawOptions);
