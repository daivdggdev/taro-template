...
import { IConfig } from '../type/canvas';
/**
 * @description 生成随机字符串
...
export function downImage(imageUrl: string) {
  const image = mapHttpToHttps(imageUrl);
  return new Promise<string>((resolve, reject) => {
    // if (/^http/.test(imageUrl) && !new RegExp(wx.env.USER_DATA_PATH).test(imageUrl))
    if (
      /^http/.test(image) &&
      !new RegExp((wx as any).env.USER_DATA_PATH).test(image) &&
      !/^http:\/\/tmp/.test(image)
    ) {
      Taro.downloadFile({
        url: image,
 ...
 * @returns  { Promise }
 */
export async function getImageInfo(imgPath: string, index: string) {
  return await Taro.getImageInfo({
    src: imgPath
  })
    .then(res => {
      return {
